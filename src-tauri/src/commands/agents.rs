...
const COLUMNS: &str = "id, name, description, system_prompt, tools, model, icon, created_at, updated_at, \
...
fn row_to_agent(r: &rusqlite::Row) -> rusqlite::Result<Agent> {
    Ok(Agent {
        ...
        icon_color: r.get(9)?, border_color: r.get(10)?,
        border_width: r.get(11)?, name_color: r.get(12)?,
        temperature: r.get(13)?, max_tokens: r.get(14)?, top_p: r.get(15)?,
    })
}
...
pub fn get_agent_by_id(conn: &rusqlite::Connection, id: &str) -> Result<Agent, String> {
    ...
    conn.query_row(&sql, [id], row_to_agent).map_err(|e| e.to_string())
}
...
pub async fn create_agent(
    ...
    description: Option<String>, system_prompt: Option<String>,
) -> Result<Agent, String> {
...
pub async fn update_agent(
    ...
    name: Option<String>, description: Option<String>, system_prompt: Option<String>,
    tools: Option<String>, model: Option<String>, icon: Option<String>,
    icon_color: Option<String>, border_color: Option<String>, border_width: Option<f64>,
    name_color: Option<String>, temperature: Option<f64>, max_tokens: Option<u32>, top_p: Option<f64>,
) -> Result<Agent, String> {
    ...
    macro_rules! set {
        ($val:expr, $col:expr) => {
            if let Some(ref v) = $val {
                conn.execute(&format!("UPDATE agents SET {}=?1, updated_at=?2 WHERE id=?3", $col), rusqlite::params![v, now, id]).map_err(|e| e.to_string())?;
                updated = true;
            }
        };
    }
    set!(name, "name"); set!(description, "description"); set!(system_prompt, "system_prompt");
    set!(tools, "tools"); set!(model, "model"); set!(icon, "icon");
    set!(icon_color, "icon_color"); set!(border_color, "border_color"); set!(border_width, "border_width");
    set!(name_color, "name_color"); set!(temperature, "temperature"); set!(max_tokens, "max_tokens"); set!(top_p, "top_p");
    if !updated { return get_agent_by_id(&conn, &id); }
    get_agent_by_id(&conn, &id)
}
