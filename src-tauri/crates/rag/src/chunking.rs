...
impl Default for ChunkConfig {
    fn default() -> Self {
        Self { chunk_size: 1000, chunk_overlap: 100 }
    }
}
...
fn split_long_para(para: &str, config: &ChunkConfig) -> Vec<String> {
    ...
    if !buf.is_empty() { result.push(buf); }
    result
}

/// 添加滑动窗口重叠（每块末尾追加下一块开头的内容）
fn add_overlap(chunks: &mut Vec<Chunk>, config: &ChunkConfig) {
    let overlap = config.chunk_overlap.min(200);
    for i in 0..chunks.len() - 1 {
        let next_start = chunks[i + 1].content.chars().take(overlap).collect::<String>();
...
mod tests {
    ...
    fn overlap_added() {
        let text = "A\n\nB\n\nC\n\nD\n\nE";
        let chunks = chunk_text("A\n\nB\n\nC\n\nD\n\nE", "doc1", "kb1", &ChunkConfig { chunk_size: 10, chunk_overlap: 5 });
