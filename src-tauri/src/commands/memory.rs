...
const STOP_WORDS: &[&str] = &[
    ...
    "to", "in", "of", "for", "on", "with", "at", "by", "from", "as", "into", "through",
    "and", "or", "but", "not", "if", "so", "than", "between", "about", "after", "before",
];
...
pub fn parse_tag_line(content: &str) -> (String, Option<String>) {
    ...
    if let Some(tag_str) = last.strip_prefix("Tag: ").or_else(|| last.strip_prefix("tag: ")) {
        let tags: Vec<(String, usize)> = tag_str.split(',')
            .filter_map(|t| {
                let t = t.trim().to_lowercase();
                if t.is_empty() { None } else { Some((t, 1)) }
            }).collect();
        if !tags.is_empty() {
            let json = serde_json::to_string(&tags.iter().map(|(t, w)| serde_json::json!({"t": t, "w": w})).collect::<Vec<_>>()).unwrap_or_default();
            let tag_idx = content.rfind("Tag:").or_else(|| content.rfind("tag:")).or_else(|| content.rfind('#')).unwrap_or(content.len());
    ...
    if let Some(tag_str) = last.strip_prefix("#tags：") {
        let tags: Vec<(String, usize)> = tag_str.split('，')
            .filter_map(|t| {
                let t = t.trim();
                if t.is_empty() { None } else { Some((t.to_lowercase(), 1)) }
            }).collect();
        if !tags.is_empty() {
            let json = serde_json::to_string(&tags.iter().map(|(t, w)| serde_json::json!({"t": t, "w": w})).collect::<Vec<_>>()).unwrap_or_default();
            let clean = content[..content.rfind('#').unwrap_or(content.len())].trim_end().to_string();
            return (clean, Some(json));
        }
    }
    ...
    if last.starts_with("#[") && last.ends_with(']') {
        ...
        let tags: Vec<(String, usize)> = inner.split(',').filter_map(|part| {
...
pub fn extract_keywords_weighted(text: &str) -> Vec<(String, usize)> {
    ...
    let stop_chars: HashSet<char> = [
        '的', '了', '是', '在', '有', '和', '就', '不', '人', '都', '一', '个', '上', '也', '很', '到',
        '说', '要', '去', '你', '会', '着', '没', '看', '好', '自', '己', '这', '他', '她', '它',
        '们', '我', '那', '对', '与', '为', '之', '及', '但', '可', '被', '从', '而', '或', '如', '因',
        '所', '其', '中', '外', '后', '前', '能', '做', '用', '让', '为',
        '该', '什', '么', '怎', '几', '哪', '谁', '何', '凡', '均', '各',
    ].iter().copied().collect();
    ...
    for c in text.chars() {
        if c.is_ascii_alphanumeric() || c == '-' || c == '_' {
            if in_cjk && !buf.is_empty() { cjk_blocks.push(buf.clone().to_lowercase()); buf.clear(); }
            buf.push(c); in_cjk = false;
        } else if c.is_alphanumeric() {
            if !in_cjk && !buf.is_empty() { eng_tokens.push(buf.clone().to_lowercase()); buf.clear(); }
            buf.push(c); in_cjk = true;
        } else {
            if !buf.is_empty() {
                if in_cjk { cjk_blocks.push(buf.clone().to_lowercase()); } else { eng_tokens.push(buf.clone().to_lowercase()); }
                buf.clear();
            }
        }
    }
    if !buf.is_empty() {
        if in_cjk { cjk_blocks.push(buf.to_lowercase()); } else { eng_tokens.push(buf.to_lowercase()); }
    }
    ...
    for block in &cjk_blocks {
        ...
        for i in 0..chars.len() {
            for len in 2..=3 {
                ...
                if gram.chars().any(|c| stop_chars.contains(&c) || c.is_ascii_digit()) { continue; }
                if added.insert(gram.clone()) { *freq.entry(gram).or_default() += 1; }
            }
        }
    }
    ...
    for t in &eng_tokens {
        ...
        if t.len() <= 1 || STOP_WORDS.contains(&t.as_str()) { continue; }
        if added.insert(t.clone()) { *freq.entry(t.clone()).or_default() += 1; }
    }
...
async fn get_api_creds(db: &ripple_conversation_store::DbPool) -> (String, String) {
    let api_key = {
        let conn = match db.get_timeout(Duration::from_secs(3)) {
            Ok(c) => c,
            Err(_) => return (String::new(), String::new()),
        };
        conn.query_row("SELECT value FROM settings WHERE key='api_key'", [], |r| r.get::<_, String>(0))
            .unwrap_or_default()
    };
    let api_base_url = {
        let conn = match db.get_timeout(Duration::from_secs(3)) {
            Ok(c) => c,
            Err(_) => return (api_key, String::new()),
        };
        conn.query_row("SELECT value FROM settings WHERE key='api_base_url'", [], |r| r.get::<_, String>(0))
...
pub async fn scan_and_index_with_db(
    ...
    let (api_key, api_base_url) = get_api_creds(db).await;
    ...
    for file in &files {
        ...
        let stored = {
            let conn = db.get_timeout(Duration::from_secs(3)).map_err(|e| e.to_string())?;
        ...
        let embeddings = match tokio::time::timeout(Duration::from_secs(30), client.embed_batch(&texts)).await {
            Ok(Ok(e)) => e,
            Ok(Err(e)) => {
        ...
        let chunk_data: Vec<(String, Option<String>, String)> = chunks
            ...
            .enumerate()
            .map(|(i, c)| {
                let emb = embeddings.get(i).map(|e| serde_json::to_string(e).unwrap_or_default());
                // 优先使用文件的 tag_line（若有）；否则提取关键词
                let tags_json = if let Some(ref tl) = tag_line { tl.clone() } else {
                    let weighted = extract_keywords_weighted(&c.content);
                    serde_json::to_string(&weighted.iter().map(|(t, w)| serde_json::json!({"t": t, "w": w})).collect::<Vec<_>>()).unwrap_or_default()
                };
        ...
        {
            let conn = db.get_timeout(Duration::from_secs(5)).map_err(|e| e.to_string())?;
            MemoryRepo::replace_file_chunks(&conn, agent_id, &rel, &hash, &chunk_data).map_err(|e| e.to_string())?;
        }
    ...
    for meta in &existing {
        if !current_paths.contains(&meta.file_path) {
            let conn = db.get_timeout(Duration::from_secs(5)).map_err(|e| e.to_string())?;
            MemoryRepo::delete_by_file(&conn, agent_id, &meta.file_path).map_err(|e| e.to_string())?;
...
/// 启动时后台索引所有 Agent 的记忆
pub async fn index_all_agents(db: ripple_conversation_store::DbPool) {
    ...
    for agent_id in agent_ids {
        match scan_and_index_with_db(&db, &agent_id).await {
            Ok(n) => tracing::info!(agent_id = %agent_id, indexed = n, "startup memory index done"),
            Err(e) => tracing::warn!(agent_id = %agent_id, error = %e, "startup memory index failed"),
        }
    }
}
...
/// 重建指定 Agent 的记忆索引
#[tauri::command]
pub async fn reindex_memories(state: State<'_, AppState>, agent_id: String) -> Result<usize, String> {
    tracing::info!(%agent_id, "reindex_memories");
    scan_and_index_with_db(&state.db, &agent_id).await
}

/// 列出 Agent 的记忆文件
#[tauri::command]
pub async fn list_memory_files(state: State<'_, AppState>, agent_id: String) -> Result<Vec<MemoryFileMeta>, String> {
...
/// 读取记忆文件内容
#[tauri::command]
pub async fn get_memory_file(_state: State<'_, AppState>, file_path: String) -> Result<String, String> {
...
/// 删除记忆文件（同时清理索引）
#[tauri::command]
pub async fn delete_memory_file(state: State<'_, AppState>, agent_id: String, file_path: String) -> Result<(), String> {
    let full = project_root().join(&file_path);
    if full.exists() {
...
/// 扫描所有 Agent 的记忆文件（用于统一记忆管理面板）
#[tauri::command]
pub async fn list_all_memory_files(_state: State<'_, AppState>) -> Result<Vec<serde_json::Value>, String> {
    let root = dailynote_dir();
    let dir_entries: Vec<String> = match std::fs::read_dir(&root) {
        Ok(entries) => entries
            .flatten()
    ...
    for dir_name in dir_entries {
        ...
        let files = scan_memory_files(&dir_path)?;
        for file in &files {
            let rel = format!("dailynote/{}/{}", dir_name, file.file_name().unwrap_or_default().to_string_lossy());
            let hash = file_hash(file).unwrap_or_default();
...
/// 保存记忆文件内容
#[tauri::command]
pub async fn save_memory_file(file_path: String, content: String) -> Result<(), String> {
    let full = project_root().join(&file_path);
    // 确保父目录存在
    ...
    tracing::info!(file = %file_path, len = content.len(), "memory file saved");
    Ok(())
}

/// 删除记忆文件（按 agent 名 + 文件路径，自动清理索引）
#[tauri::command]
pub async fn delete_agent_memory_file(
    ...
    let agent_id: Option<String> = {
        let conn = state.db.get_timeout(Duration::from_secs(5)).map_err(|e| e.to_string())?;
        conn.query_row("SELECT id FROM agents WHERE name = ?1", [&agent_name], |r| r.get(0)).ok()
    };
    if let Some(aid) = agent_id {
        let conn = state.db.get_timeout(Duration::from_secs(5)).map_err(|e| e.to_string())?;
        MemoryRepo::delete_by_file(&conn, &aid, &file_path).map_err(|e| e.to_string())?;
    }
    // 删除文件
    let full = project_root().join(&file_path);
    if full.exists() {
    ...
    tracing::info!(file = %file_path, "memory file deleted");
    Ok(())
}

/// 递归扫描 dailynote/ 下所有 .txt/.md 文件，为没有 tag 行的文件用 LLM 生成并追加 tag。
/// 失败时回退到关键词提取。
#[tauri::command]
pub async fn generate_memory_tags(state: State<'_, AppState>) -> Result<usize, String> {
    ...
    while let Some(dir) = dirs.pop() {
        if let Ok(entries) = std::fs::read_dir(&dir) {
            for entry in entries.flatten() {
                ...
                if path.is_dir() {
                    dirs.push(path);
                } else if matches!(path.extension().and_then(|s| s.to_str()), Some("txt") | Some("md")) {
                    ...
                    // LLM 生成 tag，失败时回退
                    let tags = llm_generate_tags(&api_base_url, &api_key, &llm_model, &content).await
                        .unwrap_or_else(|_| {
                            extract_keywords_weighted(&content).iter().map(|(t, _)| t.clone()).collect()
                        });
                    ...
                    f.write_all(tag_line.as_bytes()).map_err(|e| format!("write {e}"))?;
                    total += 1;
                }
            }
        }
    }
    tracing::info!(files = total, "generate_memory_tags done");
    Ok(total)
}

/// 打开 Agent 记忆目录（OS 文件管理器）
#[tauri::command]
pub async fn open_memory_dir(agent_name: String) -> Result<(), String> {
    let dir = agent_memory_dir(&agent_name);
...
/// 记忆统计
#[tauri::command]
pub async fn memory_stats(state: State<'_, AppState>, agent_id: String) -> Result<serde_json::Value, String> {
    let conn = state.db.get_timeout(Duration::from_secs(5)).map_err(|e| e.to_string())?;
...
async fn llm_generate_tags(api_base_url: &str, api_key: &str, model: &str, text: &str) -> Result<Vec<String>, String> {
    let prompt = read_prompt("tag_master.txt", "提取 5-10 个多维度关键词标签。直接输出标签，英文逗号分隔。");
    ...
    let cleaned = reply.trim().trim_start_matches("Tag:").trim_start_matches("tag:").trim();
    let tags: Vec<String> = cleaned
        .split(|c: char| c == '，' || c == ',' || c == '\n')
        .map(|t| t.trim().trim_matches('"').trim_matches('「').trim_matches('」').trim_matches('"').to_lowercase())
...
pub async fn exec_remember(
    ...
    let agent_id = {
        ...
        let metadata: String = conn
            .query_row("SELECT metadata FROM conversations WHERE id=?1", [conversation_id], |r| r.get(0))
            .map_err(|e| format!("conversation not found: {e}"))?;
        let meta: serde_json::Value = serde_json::from_str(&metadata).unwrap_or_default();
        meta.get("agent_id").and_then(|v| v.as_str()).unwrap_or("default").to_string()
    };

    // 查 agent name
    let agent_name = {
        let conn = db.get_timeout(Duration::from_secs(5)).map_err(|e| e.to_string())?;
        conn.query_row("SELECT name FROM agents WHERE id=?1", [&agent_id], |r| r.get::<_, String>(0))
    ...
    let tag_line = if weighted.is_empty() {
        ...
        format!("Tag: {}", weighted.iter().map(|(t, _w)| t.as_str()).collect::<Vec<_>>().join(", "))
    };
    {
        use std::io::Write;
        let mut f = std::fs::OpenOptions::new().append(true).open(&file).map_err(|e| format!("append tags: {e}"))?;
    ...
    let emb = tokio::time::timeout(Duration::from_secs(30), client.embed(&full_text))
        ...
        .ok()
        .and_then(|r| r.ok());
    let emb_json = emb.as_ref().map(|e| serde_json::to_string(e).unwrap_or_default());
    let tags_json = serde_json::to_string(&weighted.iter().map(|(t, w)| serde_json::json!({"t": t, "w": w})).collect::<Vec<_>>()).unwrap_or_default();
    ...
    Ok(format!("Memory stored: {}", content.chars().take(80).collect::<String>()))
}
...
pub async fn hybrid_search_memories(
    ...
    let tag_weighted = {
        let conn = db.get_timeout(Duration::from_secs(5)).map_err(|e| e.to_string())?;
        MemoryRepo::search_by_tags_weighted(&conn, agent_id, &kw_refs, top_k * 3).map_err(|e| e.to_string())?
    };
    ...
    let emb_results: Vec<(f64, MemoryChunk)> = if !all_chunks.is_empty() {
        let client = EmbeddingClient::new(api_base_url, api_key, "Qwen/Qwen3-Embedding-8B")?;
        let query_emb = match tokio::time::timeout(Duration::from_secs(15), client.embed(query)).await {
            Ok(Ok(e)) => e,
            _ => {
    ...
    for (chunk, _w) in &tag_weighted {
        if let Ok(sibs) = MemoryRepo::find_sibling_chunks(
            &db.get_timeout(Duration::from_secs(3)).map_err(|e| e.to_string())?,
            &chunk.file_path, &chunk.id,
        ) {
            for sib in sibs {
                cooc_boost.entry(sib.id.clone())
                    .and_modify(|(c, _)| *c += 1)
                    .or_insert((1, sib));
            }
        }
...
pub async fn build_memory_prompt(
    ...
    for chunk in hybrid.iter().chain(recent.iter()) {
        if !seen.insert(chunk.id.clone()) { continue; }
        let text = chunk.content.trim();
        if text.is_empty() { continue; }
        let flat = text.replace('\n', " ");
        chunks_text.push_str(&format!("- {}\n", flat));
    }
    if chunks_text.is_empty() { return None; }

    // LLM 总结（失败则回退到直接列 chunks）
    let llm_model = {
        let conn = db.get_timeout(Duration::from_secs(3)).ok();
        conn.and_then(|c| c.query_row("SELECT value FROM settings WHERE key='llm_model'", [], |r| r.get::<_, String>(0)).ok())
            .filter(|s| !s.is_empty())
            .unwrap_or_else(|| "deepseek-v4-flash".into())
    };
    let summarized = llm_summarize_memories(api_base_url, api_key, &llm_model, query, &chunks_text).await
...
async fn llm_summarize_memories(
    api_base_url: &str, api_key: &str, model: &str, query: &str, memories: &str,
) -> Result<String, String> {
    ...
    let resp = client
        ...
        .header("Authorization", format!("Bearer {api_key}"))
        .json(&body)
        .send().await.map_err(|e| format!("llm call: {e}"))?;
    if !resp.status().is_success() {
        let status = resp.status();
        let body = resp.text().await.unwrap_or_default();
        return Err(format!("llm http {status}: {body}"));
    ...
    let reply = json["choices"][0]["message"]["content"].as_str().ok_or("no response")?;
    Ok(reply.to_string())
}
